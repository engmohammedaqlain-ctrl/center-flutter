...
class HallFormScreen extends StatefulWidget {
  const HallFormScreen({super.key, this.room});
...
class _HallFormScreenState extends State<HallFormScreen> {
  ...
  late final capacity = TextEditingController(text: '${widget.room?.capacity ?? 25}');
  ...
  void _save() {
    ...
    setState(() {
      errors
        ...
        ..check('capacity', capacity.text.trim().isNotEmpty && (seats == null || seats <= 0), 'سعة غير صالحة');
    });
    if (errors.report(context)) return;
    try {
      store.upsertRoom(
      ...
      showAppSnack(context, room == null ? 'تمت إضافة القاعة' : 'تم حفظ القاعة');
      Navigator.pop(context);
    } on StoreException catch (e) {
      showAppSnack(context, e.message, error: true);
    }
  ...
  Future<void> _delete() async {
    ...
    if (used > 0) {
      showAppSnack(context, 'لا يمكن حذف القاعة لارتباطها بـ $used مجموعة', error: true);
      return;
    }
    final ok = await confirmSheet(context, title: 'حذف القاعة', message: 'حذف «${room.name}» نهائياً؟', confirmLabel: 'حذف');
    if (!ok || !mounted) return;
    try {
      store.deleteRoom(room.id);
      showAppSnack(context, 'تم حذف القاعة');
      Navigator.pop(context);
    } on StoreException catch (e) {
      showAppSnack(context, e.message, error: true);
    }
  ...
  Widget build(BuildContext context) {
    ...
    final title = editing ? 'تعديل: ${widget.room!.name}' : 'إضافة قاعة جديدة';
    if (!store.can('schedule.edit') && !store.can('settings.view')) return _denied(store, title);

    return Scaffold(
      backgroundColor: Colors.white,
      appBar: _formBar(title),
      bottomNavigationBar: FormActionBar(label: editing ? 'حفظ التعديلات' : 'إضافة القاعة', onSave: _save),
      body: GestureDetector(
        onTap: () => FocusScope.of(context).unfocus(),
        child: ListView(
          padding: const EdgeInsets.fromLTRB(16, 0, 16, 24),
          children: [
            const FormSection(icon: Icons.meeting_room_outlined, title: 'بيانات القاعة', note: 'الحقول ذات * مطلوبة'),
            FieldPair(
              startFlex: 3,
              endFlex: 2,
              start: [
                ...
                FieldLabel('السعة (مقعد)', key: errors.key('capacity')),
                TextField(
                  controller: capacity,
                  keyboardType: TextInputType.number,
                  onChanged: (_) {
                    if (errors.clear('capacity')) setState(() {});
                  },
            ...
            TextField(
              ...
              minLines: 1,
              maxLines: 3,
              decoration: const InputDecoration(hintText: 'الطابق، التجهيزات...'),
            ),
            if (editing) _DeleteButton(label: 'حذف القاعة', onPressed: _delete),
          ],
        ),
      ),
    );
